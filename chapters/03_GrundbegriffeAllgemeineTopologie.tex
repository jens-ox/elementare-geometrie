...
\begin{lemma}{Weg-Zusammenhang}
  ...
  \begin{proof}
    ...
    \textbf{Achtung}: Umkehrung gilt nicht! Z.B. ``topologische Sinuskurve'': $ X \coloneqq \left\{ \left( x, \sin\tfrac{1}{x} \right) \in \R^2 : 0 < x \leq 1 \right\} \cup \{ (0,y) : \vert y \vert < 1 \} $.\sidenote{\textbf{Details}: Singer-Thorpe p.52} $ X $ ist zusammenhängend, aber \emph{nicht} weg-zusammenhängend.
  \end{proof}
\end{lemma}
...
\begin{bla}{Test auf Homöomorphie via Zusammenhang}
...
\begin{theorem}{von Brouwer}
  ...
  \begin{proof}
    ...
    \end{equation*}
    eine Einbettung und eine injektive, stetige Abbildung von $ \R^m $ auf eine \emph{nicht} offene Teilmenge von $ \R^n $. Wäre $ \R^m \cong \R^n $, so hat man einen Widerspruch zum Satz von Gebietstreue.\sidenote{siehe auch Alexandrov-Hopf, Topologie, 1935, Kap. X.2}
  \end{proof}
\end{theorem}
...
\begin{bla}{Bemerkung --- Verwendung kompakter Räume}
...
\begin{bla}{Hinweise zur Lebesgue-Zahl}
  ...
  \begin{proof}
    \begin{enumerate}
      ...
      \item Sei $ X $ kompakt und $ A \subset X $ abgeschlossen. \\
        ...
        \end{equation*}
      \item Die allgemeine Aussage (\emph{Satz von Tichonow}\sidenote{Ist $ (X_i)_{i \in I} $ eine Familie kompakter topologischer Räume, dann ist auch das kartesische Produkt mit der Produkttopologie kompakt.}) benutzt das \emph{Lemma von Zorn}\sidenote{Eine halbgeordnete Menge, in der jede Kette eine obere Schranke hat, enthält mindestens ein maximales Element.}. \\
...
\begin{bla}{Korollar --- Min. und Max. von Teilmengen}
  Jede stetige Funktion $ f : K \to C $ auf einer kompakten Teilmenge eines Hausdorffraums nimmt ein endliches Maximum und Minimum an.\sidenote{Übungsaufgabe: Beweisen (siehe Satz von Weierstraß in Analysis)}
\end{bla}
