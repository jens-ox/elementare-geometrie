...
\begin{definition}[Topologischer Raum]\label{def:topologie}
  Ein \term{topologischer Raum} ist ein Paar \( (X, \O) \) bestehend aus einer Menge \( X \) und einem System beziehungsweise einer Familie 
  \begin{equation*}
...
\begin{example}[Standard-Topologie auf \( \R \)]\label{bsp:standardtopologie}
  \( X = \R \),
  \begin{equation*}
...
\begin{example}[Zariski-Topologie auf \( \R \)]\label{bsp:zariskitopologie}
  \( X = \R \),
  \begin{equation*}
...
\begin{definition}[Metrischer Raum \( \to \) topologischer Raum]\label{def:induzierteTopologie}
...
\begin{definition}[Basis]\label{def:basis}
  Eine \term{Basis} für die Topologie \( \O \) ist eine Teilmenge \( \mathcal{B} \subset \O \), sodass für jede offene Menge \( \varnothing \neq V \in \O \) gilt:
  \begin{equation*}
...
\begin{remark}[Gleichheit von Topologien]
...
\begin{definition}[Umgebung]\label{def:umgebung}
  \( (X, \O) \) sei ein topologischer Raum. \( U \subset X \) heißt \term{Umgebung} von \( A \subset X \), falls
  \begin{equation*}
    \exists \ O \in \O : A \subset O \subset U\text{.}
  \end{equation*}
\end{definition}

\begin{definition}[Innerer und äußerer Punkt]\label{def:innereraeussererpunkt}
  Für \( A \subset X \), \( p \in X \) heißt \( p \) ein \term{innerer Punkt} von \( A \) (bzw. \term{äußerer Punkt} von \( A \)), falls \( A \) (bzw. \( X \setminus A \)) Umgebung von \( \{ p \} \) ist. Das \term{Innere} von \( A \) ist die Menge \( \overset{\circ}{A} \) der inneren Punkte von \( A \).
\end{definition}

\begin{definition}[Abgeschlossene Hülle]
  Die \term{abgeschlossene Hülle} von \( A \) ist die Menge \( \overline{A} \subset X \), die \underline{nicht} äußere Punkte sind. \\
  \emph{Beispiel}: \( (a, b) = \{ t \in \R : a < t < b \} \), \\ \( \overline{(a,b)} = [a,b] = \{ t \in \R : a \leq t \leq b \} \).
\end{definition}

\begin{remark}[Drei konstruierte topologische Räume]
  ...
  \begin{enumerate}
    ...
    \begin{minipage}{.45\textwidth}
      \begin{figure}[H]
        \includegraphics[width=.5\textwidth]{TeilraumTopologie}
        \caption{Graphische Darstellung der Teilraum-Topologie}
      \end{figure}
    \end{minipage}

    \begin{minipage}{.45\textwidth}
      \item \term{Produkttopologie}\label{def:produkttopologie}: \( (X, \O_X) \) und \( (Y, \O_Y) \) zwei topologische Räume. Eine Teilmenge \( W \subseteq X \times Y \) ist \emph{offen} in der \emph{Produkt-Topologie} \( \Leftrightarrow \ \forall (x, y) \in W \ \exists \) Umgebung \( U \) von \( x \) in \( X \) und \( V \) von \( y \) in \( Y \) sodass das ``Kästchen'' \( U \times V \subseteq W \). \\
    ...
    \begin{minipage}{.45\textwidth}
      \begin{figure}[H]
        \includegraphics[width=\textwidth]{PunktUndKaestchen}
    ...
    \end{align*}
    \begin{figure}[H]
      \includegraphics[width=.75\textwidth]{QuotientenTopologie}
...
\begin{remark}[Hausdorffsches Trennungsaxiom \( T_2 \)]\label{def:hausdorffsch}
  Ein topologischer Raum \( (X,\O) \) heißt \term{hausdorffsch}, falls man zu je zwei verschiedenen Punkten \( p,q \in X \) disjunkte Umgebungen finden kann, also Umgebungen \( U \ni p \) und \( V \ni q \) mit \( U \cap V = \varnothing \). \\
  \emph{Beispiel}:
  \begin{enumerate}
...
\begin{definition}[Stetigkeit]\label{def:stetig}
  \( (X, \O_X) \), \( (Y, \O_Y) \) topologische Räume. Eine Abbildung \( f : X \to Y \) heißt \term{stetig}, falls die Urbilder von offenen Mengen in \( Y \) offen sind in \( X \).
\end{definition}
...
\begin{definition}[Homöomorphismus]\label{def:homoeomorphismus} \
  \begin{itemize}
    \item Eine bijektive Abbildung \( f: X \to Y \) zwischen topologischen Räumen heißt \term{Homöomorphismus}, falls \( f \) und \( f^{-1} \) stetig sind.
...
\begin{example}[Einfache Homöomorphismen]
  ...
  \begin{itemize}
    ...
    \item \( S^n = \{ x \in R^{n+1} : \Vert x \Vert^2 = 1 \} \) ist die \( n \)-dimensionale Einheitssphäre in \( \R^{n+1} \). \\
      ...
      \begin{figure}[H]
        \includegraphics[width=.5\textwidth]{StereographischeProjektion}
        \caption{Stereographische Projektion}
      \end{figure}
  \end{itemize}
\end{example}

\begin{remark}[Isometrien-Untergruppe]
...
\begin{remark}[Exkurs 1: Kurven]\label{exkurs:kurve}
  \  \\
  \begin{minipage}{.45\textwidth}
    \vspace{0.1cm}
    Was ist eine Kurve? \\
  ...
  \begin{minipage}{.45\textwidth}
    \vspace{0.1cm}  
    \begin{figure}[H]
      \includegraphics[width=.8\textwidth]{SpaceFillingCurves}
  ...
  \begin{figure}[H]
    \includegraphics[width=.5\textwidth]{Knoten}
    \caption{Äquivalente und nicht-äquivalente Knoten}
  \end{figure}
\end{remark}

\begin{remark}[Exkurs 2: Topologische Gruppen]\label{exkurs:topologischeGruppe}
  Eine \term{topologische Gruppe} ist eine Gruppe versehen mit einer Topologie, sodass die Gruppenmultiplikation
  \begin{equation*}
...
\begin{definition}[Zusammenhängend]\label{def:zusammenhaengend}
  Ein topologischer Raum \( (X, \O) \) heißt \term{zusammenhängend}, falls \( \varnothing \) und \( X \) die einzigen gleichzeitig offenen und abgeschlossenen Teilmengen von \( X \) sind. \\
...
\begin{remark}[Ergänzung: Zusammenhang von Teilmengen]
  \emph{Allgemein}: Eine Teilmenge \( B \subset X \) heißt \term{zusammenhängend}, falls sie bezüglich der Teilraumtopologie zusammenhängend ist.
\end{remark}
...
\begin{definition}[Zusammenhangskomponente]\label{def:zusammenhangskomponente}
  Sei \( x \in X \). Die \term{Zusammenhangskomponente} \( Z(x) \) ist die Vereinigung aller zusammenhängenden Teilmengen, die \( x \) enthalten.
\end{definition}
...
\begin{example}[Zusammenhangskomponenten]
  \
  \begin{enumerate}
    \item \( \R \setminus \{ t \} = \{ s \in \R : s < t \} \cup \{ s \in \R : s > t \} \) hat 2 Zusammenhangskomponenten.
    \item \( \mathbb{Q} = \R \setminus \{ \text{irrationale Zahlen} \} \) mit Teilraum-Topologie von \( (\R, \O_\text{standard}) \) ist \term{total unzusammenhängend} (alle Zusammenhangskomponenten sind einpunktig). \\
    \begin{proof}
...
\begin{definition}[Weg-Zusammenhängend]\label{def:wegzusammenhaengend}
  Sei \( (X, \O) \) ein topologischer Raum. \( X \) heißt \term{weg-zusammenhängend}, wenn es zu je zwei Punkten \( p, q \in X \) einen Weg (also eine stetige Abbildung \( \alpha : [0,1] \to X \) mit \( \alpha(0) = p \) und \( \alpha(1) = q \)) zwischen \( p \) und \( q \) gibt.
  \begin{figure}[H]
    \includegraphics[width=.5\textwidth]{Wegzusammenhang}
...
\begin{lemma}[Weg-Zusammenhang]
  ...
  \begin{minipage}{.45\textwidth}
    \begin{figure}[H]
      \includegraphics[width=.8\textwidth]{TopologischeSinuskurve}
...
\begin{definition}[(Lokal) kompakt]\label{def:kompakt}
  Ein topologischer Raum heißt \term{kompakt}, wenn jede offene Überdeckung von \( X \) eine \emph{endliche} Teilüberdeckung besitzt, also
  \begin{align*}
...
\begin{remark}[Verwendung kompakter Räume]
  ...
  \begin{figure}[H]
    \includegraphics[width=.5\textwidth]{KompakterRaumLokalGlobal}
    \caption{Konstruktion zum Schließen von lokalen auf globale Eigenschaften bei kompakten Räumen}
  \end{figure}
\end{remark}

\begin{example}[Beschränktheit im Kompakten]\label{bsp:beschraenkt}
  Ist \( X \) kompakt und \( f: X \to \R \) \term{lokal beschränkt} (also hat jeder Punkt von \( X \) eine Umgebung, in der \( f \) beschränkt ist --- beispielsweise wahr für stetige Funktionen), dann ist \( f \) beschränkt.
\end{example}

\begin{example}[Kompaktheit von Intervallen]
  \( I = [0,1] \) ist kompakt (ebenso \( [a,b] \)).
  \begin{proof}
    Sei \( {(U_i)}_{i \in I} \) eine offene Überdeckung von \( [0,1] \). Dann existiert eine sogenannte \term{Lebesgue-Zahl}\label{def:lebesgueZahl} \( \delta > 0 \), sodass jedes Teilintervall \( I_\delta \subset I \) der Länge \( \delta \) in einem \( U_i \) liegt. Da \( [0,1] \) mit endlich vielen Intervallen der Länge \( \delta \) überdeckt werden kann, kann man das auch mit endlich vielen \( U_i \). \qed{}
  \end{proof}
\end{example}

\begin{remark}[Hinweise zur Lebesgue-Zahl]
...
\begin{example}[Weitere kompakte Mengen]
  \
  \begin{enumerate}
...
\begin{lemma}[Kompakte Mengen in Hausdorffraum abgeschlossen]
...
\begin{theorem}[Homöomorphismen auf Hausdorff-Räumen]
