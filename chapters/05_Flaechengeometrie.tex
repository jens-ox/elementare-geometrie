...
\begin{theorem}[Satz von Gauß-Bonnet --- lokale Version]
  ...
  Sei $ G \subseteq x(U) \subset S $ ein einfach zusammenhängendes Gebiet mit differenzierbarem Rand $ \delta(G) $. Dann gilt:
  \begin{equation*}
      \int_{\delta(G)}\kappa_g(s)\text{d}s + \iint_G K\text{d}A = 2\pi\text{,}
    \end{equation*}  
    wobei
    \begin{align*}
      \iint_G K\text{d}A &= \iint_{x^{-1}(G)}\kappa\sqrt{EG - F^2}\text{d}u\text{d}v\text{,} \\
      \int_{\delta(G)}\kappa_g(s)\text{d}s &= \int_c \kappa_g(s)\text{d}s\text{,}
    \end{align*}
    wobei $ c $ positive\footnote{also $ x^{-1}\circ c $ läuft gegen den Uhrzeigersinn} Parametrisierung von $ \delta(G) $ nach Bogenlänge ist.
\end{theorem}

\begin{remark}[Beweisvorbereitungen]
  \
  \begin{enumerate}
    \item \emph{Lemma 1} (Formel von Stokes). Sei $ \widetilde{G} $ ein ebenes Gebiet mit differenzierbarem Rand $ \delta\widetilde{G} $. Seien $ P,Q : \widetilde{G} \to \R $ differenzierbar. Dann:
    \begin{equation*}
