...
\begin{figure}[H]
  \includegraphics{TopologischAequivalent}
...
\begin{definition}[Metrischer Raum]\label{def:metrischerRaum}
  Ein \term{metrischer Raum} ist ein Paar \( (X,d) \) aus einer Menge und einer Metrik auf dieser.
\end{definition}

\begin{definition}[Pseudometrik]\label{def:pseudometrik}
  Eine \term{Pseudometrik} erfüllt die gleichen Bedingungen wie eine Metrik, außer \( d(x,y) = 0 \Rightarrow x = y \) --- die Umkehrung gilt.
\end{definition}

\begin{definition}[Abgeschlossener \( r \)-Ball um \( x \)]\label{def:abgeschlossenerBall}
  Eine Teilmenge
  \begin{equation*}
...
\begin{definition}[Abstandserhaltende Abbildung]\label{def:abstandserhaltendeAbbildung}
  Sind \( (X, d_X) \) und \( (Y, d_Y) \) metrische Räume, so heißt eine Abbildung \( f: X \to Y \) \term{abstandserhaltend}, falls
  \begin{equation*}
...
\begin{example}[Triviale Metrik]\label{bsp:trivialeMetrik}
  Menge \( X \),
  \begin{equation*}
  ...
  \end{cases}\text{,}
  \end{equation*}
  also lässt sich mithilfe der \term{trivialen Metrik} jede Menge zu einem metrischen Raum verwursten. 
\end{example}

\begin{example}[Simple Metriken]\label{bsp:simpleMetriken}
...
\begin{example}[Euklidische Standardmetrik]\label{bsp:standardmetrik}
  \( X = \R^n \),
  \begin{equation*}
...
\begin{example}[Standardmetrik und Maximumsmetrik allgemein: Norm]\label{bsp:norm}
...
\begin{example}[Einheitssphäre]\label{bsp:einheitssphaere}
  ...
  \begin{figure}[H]
    \includegraphics[width=.5\textwidth]{Einheitssphaeren}
